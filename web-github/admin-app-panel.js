...
// v12 rule: the panel only accepts email+password sessions — Google/OAuth
// sessions are signed out with a notice, so regular users can never use it.
...
async function session(sessionData) {
  ...
  if (!user) return;
  // RULE: the panel only accepts email+password sessions. A session opened
  // through Google (the app login) is rejected, so regular users — and even
  // an admin signed in via Google — must use the dedicated email+password
  // credentials here.
  if (user.app_metadata?.provider && user.app_metadata.provider !== 'email') {
    await db.auth.signOut();
    return note('ورود به پنل فقط با ایمیل و رمز مجاز است. ورود با گوگل در پنل پذیرفته نمی‌شود.', true);
  }
